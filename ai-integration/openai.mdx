...
Here's a complete example showing how to create a payment-enabled chatbot using OpenAI and PayLink:
...
async def main():
    ...
    # Get PayLink tools in OpenAI format
    tools = await paylink_client.list_tools()
    
    # User message requesting payment
    messages = [
        {
            "role": "system",
            "content": "You are a helpful assistant that can process mobile money payments when requested."
        },
        {
            "role": "user",
            "content": "I want to pay 1 KES from my phone number 2547123456789 for invoice number 123."
        }
    ]
    
    # Step 1: Get the initial response from OpenAI with tool calls
    ...
    # Step 2: Check if there's a tool call to process
    if assistant_message.tool_calls:
        ...
        for tool_call in assistant_message.tool_calls:
            ...
            # Execute the PayLink tool
            tool_result = await paylink_client.call_tool(function_name, function_args)
        ...
        # Step 3: Get the final response from OpenAI
        ...
        print("No payment processing needed")
